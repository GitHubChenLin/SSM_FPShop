...
<script>
	function confirmRec(oid) {
		if(confirm("是否确认收货")){
			location.href = "${pageContext.request.contextPath}/confirmRec?oid="+oid; 
		}
	}
	function payOrder(oid){
		parent.location.href = "${pageContext.request.contextPath}/payUnpayOrder?oid="+oid;
	}
</script>
<style>
a,a:hover{
					...
					<tr>
						...
						<td rowspan="${order.orderItems.size()}"
							data-state="${order.state }"><c:choose>
								<c:when test="${order.state == 0 }"><span class="action"><a href="javascript:;"
										onclick="payOrder('${order.oid}')">未付款</a></span></c:when>
