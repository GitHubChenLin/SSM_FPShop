...
<script>
	function delCartItem(pid) {
		if (confirm("您是否要删除该项？")) {
			location.href = "${pageContext.request.contextPath }/delCartItem?pid="
					+ pid;
		}
	}
	...
	function clearCart() {
		if (confirm("您是否要清空购物车？")) {
			location.href = "${pageContext.request.contextPath }/removeCart";
		}
	}
	function setDefault(aid) {
		location.href = "${pageContext.request.contextPath}/user/setDefaultAddFromCart?aid=" + aid;
		/* $.ajax({
			url : "${pageContext.request.contextPath}/user/setDefaultAddFromCart",
			data : {
				"aid" : aid
			},
			async : true,
