...
<body>
	<div id = "aaa"><jsp:include page="/WEB-INF/jsp/header.jsp"></jsp:include></div>
	<div id = "body" style = "">
		<div class="product-list" data-cid = "">
			<c:forEach var="product" items="${pageBean.list }">
				<div class="product-item1">
					<a href="${pageContext.request.contextPath }/product?pid=${product.pid}&cid=${product.cid}"
					...
					<div style=""><a href="${pageContext.request.contextPath }/product?pid=${product.pid}&cid=${product.cid}">${product.pname }</a></div>
				</div>
			</c:forEach>
		</div>
		<div class="" style="width: 100%;height:auto; margin: 0 auto; margin-top: 20px;display:block;">
			<ul class="pagination" style="">
				...
				<c:if test="${pageBean.currentPage!=1}">
					<li><a href="${pageContext.request.contextPath }/productByCategory?cid=${cid}&currentPage=${pageBean.currentPage-1}" style = "margin-right:5px;">◀</a></li>	
				</c:if>
				...
				<c:forEach var = "page" begin="1" end = "${pageBean.totalPage }">
					...
					<c:if test="${page != pageBean.currentPage }">
						<li><a href="${pageContext.request.contextPath }/productByCategory?cid=${cid}&currentPage=${page}">${page }</a></li>
					</c:if>
				</c:forEach>
				...
				<c:if test="${pageBean.currentPage!=pageBean.totalPage}">
					<li><a href="${pageContext.request.contextPath }/productByCategory?cid=${cid}&currentPage=${pageBean.currentPage+1}" style = "margin-left:5px;">▶</a></li>	
				</c:if>
			</ul>
		</div>
	</div>
